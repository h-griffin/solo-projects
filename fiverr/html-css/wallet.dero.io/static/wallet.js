...
var wallet_error = ""; // tracks registrations
...
worker.addEventListener('message', function(e) {
    ...
    case "stats":
        ...
        wallet_error = e.data.wallet_error;
        break;
    };
}, false);
...
function update_balance () {
    ...
    if ( wallet_error == ""){
        document.querySelectorAll('#register').forEach(function(node) {
            node.style.display = "none";
        });
        document.querySelectorAll('#registernav').forEach(function(node) {
            node.style.display = "none";
        });
    }else{
        document.querySelectorAll('#register').forEach(function(node) {
                ...
                if ( response.error == "" ) {
                   ...
                   //     return;
                    ...
                    verify_password_action( function() {
                        create_transaction_verify_send(element_address.value,element_amount.value,"");
                    } );
                } else {
