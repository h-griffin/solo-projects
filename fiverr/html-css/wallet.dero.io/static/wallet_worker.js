...
db.wallets.put({name: "John Doe", edb: "hello"}).then (function() {
...
var go = new Go();
var mod = fetchAndInstantiate("/webwallet.wasm", go.importObject);
...
registerPromiseWorker(async function (message) {
    ...
    case  "create_new": // recover wallet from key
        error_message = "";
        return_value = DERO_JS_Create_New_Wallet('',message.password);
    ...
    case  "register_wallet": // close wallet
        DERO_JS_Register();
        break;
...
async   function save_wallet () {
    ...
    if (wallet_local_name.length >= 1) {
        console.log("saving dump  " + wallet_local_name)
